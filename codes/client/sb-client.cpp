...
create_overlay(struct RTSPThreadParam *rtspParam, int ch) {
	...
	if(rtspParam->surface[ch] != NULL) {
		...
		rtsperror("client: duplicated create window request - image comes too fast?\n");
		return;
	}
	...
	for(data = pipe->in; data != NULL; data = data->next) {
		...
		if(avpicture_alloc((AVPicture*) data->pointer, AV_PIX_FMT_YUV420P, w, h) != 0) {
			rtsperror("client: per frame initialization failed.\n");
			exit(-1);
		}
	}
	// sdl
	...
	if(sb_conf_readbool("fullscreen", 0) != 0) {
		wflag |= SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_BORDERLESS;
	}
#endif
...
#endif
	if(surface == NULL) {
		rtsperror("client: set video mode (create window) failed.\n");
		exit(-1);
	}
	...
	if(relativeMouseMode != 0) {
		...
		SDL_SetRelativeMouseMode(SDL_TRUE);
#endif		////
		sb_error("client: relative mouse mode enabled.\n");
	}
	//
#if 1	// only support SDL2
	do {	// choose SW or HW renderer?
		...
		sb_conf_readv("video-renderer", renderer_name, sizeof(renderer_name));
		if(strcmp("software", renderer_name) == 0) {
			rtsperror("client: configured to use software renderer.\n");
		...
		for(i = 0; i < n; i++) {
			...
			if(strcmp(renderer_name, info.name) == 0)
				renderer_index = i;
			rtsperror("client: renderer#%d - %s (%s%s%s%s)%s\n",
				i, info.name,
	...
	if(renderer == NULL) {
		rtsperror("client: create renderer failed.\n");
		exit(-1);
	}
	//
...
#endif
	if(overlay == NULL) {
		rtsperror("client: create overlay (textuer) failed.\n");
		exit(-1);
	}
	//
	pthread_mutex_lock(&rtspParam->surfaceMutex[ch]);
	...
	pthread_mutex_unlock(&rtspParam->surfaceMutex[ch]);
	//
	rtsperror("client: window created successfully (%dx%d).\n", w, h);
	// initialize watchdog
...
open_audio(struct RTSPThreadParam *rtspParam, AVCodecContext *adecoder) {
	...
	if(rtspconf->audio_device_format == AV_SAMPLE_FMT_S16) {
		...
		rtsperror("client: open audio - unsupported audio device format.\n");
		return;
	}
	...
	if(SDL_OpenAudio(&wanted, &spec) < 0) {
		pthread_mutex_unlock(&rtspParam->audioMutex);
		rtsperror("client: open audio failed - %s\n", SDL_GetError());
		return;
	}
	//
	...
	SDL_PauseAudio(0);
	pthread_mutex_unlock(&rtspParam->audioMutex);
	rtsperror("client: audio device opened.\n");
	return;
}
...
render_image(struct RTSPThreadParam *rtspParam, int ch) {
	...
	if(SDL_LockTexture(rtspParam->overlay[ch], NULL, (void**) &pixels, &pitch) == 0) {
		...
		rtsperror("client: lock textture failed - %s\n", SDL_GetError());
	}
#endif
...
ProcessEvent(SDL_Event *event) {
	...
	case SDL_KEYUP:
		...
		if(savefp_keyts != NULL) {
			...
			sb_save_printf(savefp_keyts, "KEY-UP: %u.%06u scan 0x%04x sym 0x%04x mod 0x%04x\n",
	...
	case SDL_KEYDOWN:
		...
		if(savefp_keyts != NULL) {
			gettimeofday(&tv, NULL);
			sb_save_printf(savefp_keyts, "KEY-DN: %u.%06u scan 0x%04x sym 0x%04x mod 0x%04x\n",
				tv.tv_sec, tv.tv_usec,
				event->key.keysym.scancode,
				event->key.keysym.sym,
				event->key.keysym.mod);
...
main(int argc, char *argv[]) {
	...
	//
#ifdef ANDROID
	if(sb_init("/sdcard/ga/android.conf", NULL) < 0) {
	...
	sb_openlog();
	//
	if(sb_conf_readbool("control-relative-mouse-mode", 0) != 0) {
		rtsperror("*** Relative mouse mode enabled.\n");
		relativeMouseMode = 1;
	}
	//
	if(sb_conf_readv("save-key-timestamp", savefile_keyts, sizeof(savefile_keyts)) != NULL) {
		savefp_keyts = sb_save_init_txt(savefile_keyts);
	...
	if(rtspconf->video_renderer_software == 0) {
		sb_error("SDL: prefer opengl hardware renderer.\n");
	...
	if(sb_conf_readbool("enable-watchdog", 1) == 1) {
