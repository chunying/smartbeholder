...
{
	sdlmsg_keyboard_t *msgk = (sdlmsg_keyboard_t*) msg;
	//sb_error("sdl client: key event code=%x key=%x mod=%x pressed=%u\n", scancode, key, mod, pressed);
...
sdlmsg_mousekey(sdlmsg_t *msg, unsigned char pressed, unsigned char button, unsigned short x, unsigned short y) {
	sdlmsg_mouse_t *msgm = (sdlmsg_mouse_t*) msg;
	//sb_error("sdl client: button event btn=%u pressed=%u\n", button, pressed);
...
sdlmsg_mousewheel(sdlmsg_t *msg, unsigned short mousex, unsigned short mousey) {
	sdlmsg_mouse_t *msgm = (sdlmsg_mouse_t*) msg;
	//sb_error("sdl client: motion event x=%u y=%u\n", mousex, mousey);
	bzero(msg, sizeof(sdlmsg_mouse_t));
	msgm->msgsize = htons(sizeof(sdlmsg_mouse_t));
...
sdlmsg_replay_init(void *arg) {
	...
	if(rect != NULL) {
		if(sb_fillrect(&croprect, rect->left, rect->top, rect->right, rect->bottom) == NULL) {
			sb_error("controller: invalid rect (%d,%d)-(%d,%d)\n",
				rect->left, rect->top,
				rect->right, rect->bottom);
			return -1;
		}
		prect = &croprect;
		sb_error("controller: crop rect (%d,%d)-(%d,%d)\n",
	...
	//
	sb_error("sdl_replayer: sizeof(sdlmsg) = %d\n", sizeof(sdlmsg_t));
	//
#ifdef WIN32
	...
	sb_error("sdl replayer: Replay using SendInput(), screen-size=%dx%d\n", cxsize, cysize);
#elif defined __APPLE__
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    sb_error("sdl replayer: iOS is not supported now\n");
#else
	do {
	...
	ctrl_server_set_resolution(cxsize, cysize);
	ctrl_server_set_output_resolution(cxsize, cysize);
	sb_error("sdl replayer: Replay using CGPostEvent(), screen-size=%dx%d\n",
		cxsize, cysize);
#endif
#elif defined ANDROID
	sb_error("sdl replayer: Android is not supported now\n");
#else	// X11
	int evtbase, errbase, major, minor;
	if((display = XOpenDisplay(conf->display)) == NULL) {
		sb_error("sdl replayer: cannot open display '%s'\n",
			conf->display);
		return -1;
	}
	//
	...
	if(XTestQueryExtension(display, &evtbase, &errbase, &major, &minor) == False) {
		sb_error("sdl replayer: XTest not supported.\n");
		return -1;
	}
	sb_error("sdl replayer: Replay using XTest (version %d.%d) for display %s screen %d, size=%dx%d.\n",
	...
	do {
		int resolution[2];
		int baseX, baseY;
		if(sb_conf_readints("output-resolution", resolution, 2) != 2)
			break;
		//
		...
		//
		sb_error("sdl replayer: mouse coordinate scale factor = (%.3f,%.3f)\n",
...
sdlmsg_replay_native(sdlmsg_t *msg) {
	...
	case SDL_EVENT_MSGTYPE_KEYBOARD:
		...
		if((in.ki.wVk = SDLKeyToKeySym(msgk->sdlkey)) != INVALID_KEY) {
			...
			//sb_error("sdl replayer: vk=%x scan=%x\n", in.ki.wVk, in.ki.wScan);
			SendInput(1, &in, sizeof(in));
		} else {
		////////////////
		sb_error("sdl replayer: undefined key scan=%u(%04x) key=%u(%04x) mod=%u(%04x) pressed=%d\n",
		...
		break;
	case SDL_EVENT_MSGTYPE_MOUSEKEY:
		//sb_error("sdl replayer: button event btn=%u pressed=%d\n", msg->mousebutton, msg->is_pressed);
		bzero(&in, sizeof(in));
		in.type = INPUT_MOUSE;
	...
	case SDL_EVENT_MSGTYPE_MOUSEWHEEL:
		...
		break;
	case SDL_EVENT_MSGTYPE_MOUSEMOTION:
		//sb_error("sdl replayer: motion event x=%u y=%d\n", msgm->mousex, msgm->mousey);
		bzero(&in, sizeof(in));
		in.type = INPUT_MOUSE;
...
#if 0
		sb_error("sdl replayer: received key scan=%u(%04x) key=%u(%04x) mod=%u(%04x) pressed=%d\n",
...
sdlmsg_kb_init() {
	...
	kbSdlkey.clear();
	// load scancodes
	if(sb_conf_mapsize("key-block-scancode") > 0) {
		sb_conf_mapreset("key-block-scancode");
		for(	key = sb_conf_mapkey("key-block-scancode", keybuf, sizeof(keybuf));
			key != NULL;
			key = sb_conf_mapnextkey("key-block-scancode", keybuf, sizeof(keybuf))) {
			//
			unsigned short sc = strtol(key, NULL, 0);
			if(sc == 0)
				continue;
			//
			val = sb_conf_mapvalue("key-block-scancode", valbuf, sizeof(valbuf));
			if(val == NULL)
				continue;
			//
			if(sb_conf_boolval(val, 0) != 0)
				kbScancode[sc] = 1;
		}
	}
	// load sdlkey
	if(sb_conf_mapsize("key-block-keycode") > 0) {
		sb_conf_mapreset("key-block-keycode");
		for(	key = sb_conf_mapkey("key-block-keycode", keybuf, sizeof(keybuf));
			key != NULL;
			key = sb_conf_mapnextkey("key-block-keycode", keybuf, sizeof(keybuf))) {
			//
			unsigned short kc = strtol(key, NULL, 0);
			if(kc == 0)
				continue;
			//
			val = sb_conf_mapvalue("key-block-keycode", valbuf, sizeof(valbuf));
			if(val == NULL)
				continue;
			//
			if(sb_conf_boolval(val, 0) != 0)
	...
	//
	sb_error("key-blocking initialized: %u+%u keys blocked.\n",
...
sdlmsg_replay_callback(void *msg, int msglen) {
	...
	if(msglen != ntohs(m->msgsize)/*sizeof(sdlmsg_t)*/) {
		sb_error("message length mismatched. (%d != %d)\n",
