...
packet_queue_init(PacketQueue *q) {
	...
	if(sb_conf_readv("audio-playback-queue-limit", buf, sizeof(buf)) != NULL) {
		// must ensure that we have configured audio-playback-queue-limit
		if((val = sb_conf_readint("audio-playback-queue-limit")) >= 0) {
			packet_queue_limit = val;
		}
	}
	if((val = sb_conf_readint("audio-playback-queue-dropfactor")) > 0) {
		packet_queue_dropfactor = val;
	}
	sb_error("packet queue: initialized - limit %d%s; dropfactor %d%s\n",
		packet_queue_limit,
...
rtsperror(const char *fmt, ...) {
	...
	va_start(ap, fmt);
#ifdef ANDROID
	__android_log_vprint(ANDROID_LOG_INFO, "sb_log", fmt, ap);
#else
...
init_vdecoder(int channel, const char *sprop) {
	...
	if((names = sb_lookup_ffmpeg_decoders(video_codec_name)) == NULL) {
		rtsperror("video decoder: cannot find decoder names for %s\n", video_codec_name);
		return -1;
	}
	video_codec_id = sb_lookup_codec_id(video_codec_name);
	if((codec = sb_avcodec_find_decoder(names, AV_CODEC_ID_NONE)) == NULL) {
...
init_adecoder() {
	...
	if((names = sb_lookup_ffmpeg_decoders(audio_codec_name)) == NULL) {
		rtsperror("audio decoder: cannot find decoder names for %s\n", audio_codec_name);
		return -1;
	}
	audio_codec_id = sb_lookup_codec_id(audio_codec_name);
	if((codec = sb_avcodec_find_decoder(names, AV_CODEC_ID_NONE)) == NULL) {
	...
	if((seq-1) == mi->second.lastPktSeq) {
		if(sampleframe) {
			...
			if(mi->second.framecount >= 240 && mi->second.samples >= 3000) {
				...
				sb_error("XXX: received: %uKB; capacity = %.3fKbps (%d samples); loss-rate = %.2f%% (%u/%u); average per-frame overhead factor = %.2f\n",
...
rtp_packet_handler(void *clientData, unsigned char *packet, unsigned &packetSize) {
	...
	if(log_rtp > 0) {
#ifdef ANDROID
		sb_log("%10u.%06u log_rtp: flags %04x seq %u ts %u ssrc %u size %u\n",
			tv.tv_sec, tv.tv_usec, flags, seqnum, timestamp, ssrc, packetSize);
#else
		sb_log("log_rtp: flags %04x seq %u ts %u ssrc %u size %u\n",
...
drop_video_frame_init(long long max_delay_us) {
	...
	if(max_tolerable_video_delay_us > 0) {
		sb_error("rtspclient: max tolerable video delay = %lldus\n", max_tolerable_video_delay_us);
	} else {
		sb_error("rtspclient: max tolerable video delay disabled.\n");
	}
	return;
}
...
drop_video_frame(int ch/*channel*/, unsigned char *buffer, int bufsize, struct timeval pts) {
	...
	if(drop_vframe_ctx[ch].tv_real_start.tv_sec == 0) {
		...
		sb_error("rtspclient: frame dropping initialized real=%lu.%06ld stream=%lu.%06ld (latency=%lld).\n",
	...
	do {
		if(video_codec_id == AV_CODEC_ID_H264) {
			...
			if(dreal-dstream > max_tolerable_video_delay_us) {
				if(drop_vframe_ctx[ch].no_drop > 0)
					break;
				sb_error("drop_frame: packet dropped (delay=%lldus)\n", dreal-dstream);
				return 1;
			}
		}
	} while(0);
...
play_video_priv(int ch/*channel*/, unsigned char *buffer, int bufsize, struct timeval pts) {
	...
	if(btv0.tv_sec == 0) {
		...
		if(dt < 2000000) {
			sb_aggregated_print(0x8000, 599, dt);
		}
		btv0 = btv1;
	}
#endif
	...
	do {
		...
		for(	ptr = sb_find_startcode(avpkt.data, avpkt.data+avpkt.size, &codelen);
			ptr != NULL;
			ptr = sb_find_startcode(ptr+codelen, avpkt.data+avpkt.size, &codelen)) {
			//
		...
		if(got_picture) {
			...
			&& vframe[ch]->format == rtspParam->format[ch]) {
				...
				if((swsctx = create_frame_converter(
					...
					sb_error("*** FATAL *** Create frame converter failed.\n");
#ifdef ANDROID
			...
			if(ch==0 && savefp_yuv != NULL) {
				sb_save_yuv420p(savefp_yuv, vframe[0]->width, vframe[0]->height, dstframe->data, dstframe->linesize);
				if(savefp_yuvts != NULL) {
					gettimeofday(&ftv, NULL);
					sb_save_printf(savefp_yuvts, "Frame #%08d: %u.%06u\n", fcount++, ftv.tv_sec, ftv.tv_usec);
				}
			}
			...
			sb_aggregated_print(0x8001, 601, tvdiff_us(&ptv1, &ptv0));
#endif
#ifdef ANDROID
...
rtsp_thread(void *param) {
	...
	sb_aggregated_reset();
	drop_video_frame_init(sb_conf_readint("max-tolerable-video-delay"));
	// save-file features
	if(savefp_yuv != NULL)
		sb_save_close(savefp_yuv);
	if(savefp_yuvts != NULL)
		sb_save_close(savefp_yuvts);
	savefp_yuv = savefp_yuvts = NULL;
	//
	if(sb_conf_readbool("log-rtp-packet", 0) != 0)
		log_rtp = 1;
	if(sb_conf_readv("save-yuv-image", savefile_yuv, sizeof(savefile_yuv)) != NULL)
		savefp_yuv = sb_save_init(savefile_yuv);
	if(savefp_yuv != NULL
	&& sb_conf_readv("save-yuv-image-timestamp", savefile_yuvts, sizeof(savefile_yuvts)) != NULL)
		savefp_yuvts = sb_save_init_txt(savefile_yuvts);
	rtsperror("*** SAVEFILE: YUV image saved to '%s'; timestamp saved to '%s'.\n",
		savefp_yuv   ? savefile_yuv   : "NULL",
		savefp_yuvts ? savefile_yuvts : "NULL");
	//
	if(sb_conf_readint("rtp-reordering-threshold") > 0) {
		rtp_packet_reordering_threshold = sb_conf_readint("rtp-reordering-threshold");
	}
		...
		if (!scs.subsession->initiate()) {
			...
			if(strcmp("video", scs.subsession->mediumName()) == 0) {
				...
				if(rtp_packet_reordering_threshold > 0)
					scs.subsession->rtpSource()->setPacketReorderingThresholdTime(rtp_packet_reordering_threshold);
#ifdef ANDROID
				if((mime = sb_lookup_mime(audio_codec_name)) == NULL) {
...
#if 0
			if(lost > 0) {
				sb_error("rtspclient: frame corrupted? lost=%d; count=%d (packets)\n", lost, count);
			}
#endif
		}
		//
