...
rtspconf_load_codec(const char *key, const char *value,
	...
	if((token = strtok_r(buf, DELIM, &saveptr)) == NULL) {
		sb_error("# RTSP[config]: no codecs specified for %s.\n", key);
		return -1;
	}
	do {
	...
	if((*codec = finder(names, AV_CODEC_ID_NONE)) == NULL) {
		sb_error("# RTSP[config]: no available %s codecs (%s).\n", key, value);
		return -1;
	}
	//
	sb_error("# RTSP[config]: %s = %s (%s)\n", key, (*codec)->name,
...
rtspconf_parse(struct RTSPConf *conf) {
	...
	rtspconf_init(conf);
	//
	if((ptr = sb_conf_readv("server-name", buf, sizeof(buf))) != NULL) {
		conf->servername = strdup(ptr);
	}
	//
	if((ptr = sb_conf_readv("base-object", buf, sizeof(buf))) != NULL) {
		strncpy(conf->object, ptr, RTSPCONF_OBJECT_SIZE);
	}
	//
	if((ptr = sb_conf_readv("title", buf, sizeof(buf))) != NULL) {
		strncpy(conf->title, ptr, RTSPCONF_TITLE_SIZE);
	}
	//
	if((ptr = sb_conf_readv("display", buf, sizeof(buf))) != NULL) {
		strncpy(conf->display, ptr, RTSPCONF_DISPLAY_SIZE);
	}
	//
	v = sb_conf_readint("server-port");
	if(v <= 0 || v >= 65536) {
		sb_error("# RTSP[config]: invalid server port %d\n", v);
		return -1;
	}
	conf->serverport = v;
	//
	ptr = sb_conf_readv("proto", buf, sizeof(buf));
	if(ptr == NULL || strcmp(ptr, "tcp") != 0) {
		conf->proto = IPPROTO_UDP;
		sb_error("# RTSP[config]: using 'udp' for RTP flows.\n");
	} else {
		conf->proto = IPPROTO_TCP;
		sb_error("# RTSP[config]: using 'tcp' for RTP flows.\n");
	}
	//
	conf->ctrlenable = sb_conf_readbool("control-enabled", 0);
	//
	if(conf->ctrlenable != 0) {
		//
		v = sb_conf_readint("control-port");
		if(v <= 0 || v >= 65536) {
			sb_error("# RTSP[config]: invalid control port %d\n", v);
			return -1;
		}
		conf->ctrlport = v;
		sb_error("# RTSP[config]: controller port = %d\n", conf->ctrlport);
		//
		ptr = sb_conf_readv("control-proto", buf, sizeof(buf));
		if(ptr == NULL || strcmp(ptr, "tcp") != 0) {
			conf->ctrlproto = IPPROTO_UDP;
			sb_error("# RTSP[config]: controller via 'udp' protocol.\n");
		} else {
			conf->ctrlproto = IPPROTO_TCP;
			sb_error("# RTSP[config]: controller via 'tcp' protocol.\n");
		}
		//
		conf->sendmousemotion  = sb_conf_readbool("control-send-mouse-motion", 1);
	}
	// video-encoder, audio-encoder, video-decoder, and audio-decoder
	if((ptr = sb_conf_readv("video-encoder", buf, sizeof(buf))) != NULL) {
		if(rtspconf_load_codec("video-encoder", ptr,
			(const char**) conf->video_encoder_name,
			&conf->video_encoder_codec,
			sb_avcodec_find_encoder) < 0)
			return -1;
	}
#if 0
	if((ptr = sb_conf_readv("video-decoder", buf, sizeof(buf))) != NULL) {
		if(rtspconf_load_codec("video-decoder", ptr,
			(const char**) conf->video_decoder_name,
			&conf->video_decoder_codec,
			sb_avcodec_find_decoder) < 0)
			return -1;
	}
#endif
	if((ptr = sb_conf_readv("audio-encoder", buf, sizeof(buf))) != NULL) {
...
#if 0
	if((ptr = sb_conf_readv("audio-decoder", buf, sizeof(buf))) != NULL) {
...
#endif
	//
	v = sb_conf_readint("video-fps");
	if(v <= 0 || v > 120) {
		sb_error("# RTSP[conf]: video-fps out-of-range %d (valid: 1-120)\n", v);
		return -1;
	}
	conf->video_fps = v;
	//
	ptr = sb_conf_readv("video-renderer", buf, sizeof(buf));
	...
	//
	v = sb_conf_readint("audio-bitrate");
	if(v <= 0 || v > 1024000) {
		sb_error("# RTSP[config]: audio-bitrate out-of-range %d (valid: 1-1024000)\n", v);
		return -1;
	}
	conf->audio_bitrate = v;
	//
	v = sb_conf_readint("audio-samplerate");
	if(v <= 0 || v > 1024000) {
		sb_error("# RTSP[config]: audio-samplerate out-of-range %d (valid: 1-1024000)\n", v);
		return -1;
	}
	conf->audio_samplerate = v;
	//
	v = sb_conf_readint("audio-channels");
	if(v < 1) {
		sb_error("# RTSP[config]: audio-channels must be greater than zero (%d).\n", v);
		return -1;
	}
	conf->audio_channels = v;
	//
	if((ptr = sb_conf_readv("audio-device-format", buf, sizeof(buf))) == NULL) {
		sb_error("# RTSP[config]: no audio device format specified.\n");
		return -1;
	}
	if((conf->audio_device_format = av_get_sample_fmt(ptr)) == AV_SAMPLE_FMT_NONE) {
		sb_error("# RTSP[config]: unsupported audio device format '%s'\n", ptr);
		return -1;
	}
	//
	if((ptr = sb_conf_readv("audio-device-channel-layout", buf, sizeof(buf))) == NULL) {
		sb_error("# RTSP[config]: no audio device channel layout specified.\n");
		return -1;
	}
	if((conf->audio_device_channel_layout = av_get_channel_layout(ptr)) == 0) {
		sb_error("# RTSP[config]: unsupported audio device channel layout '%s'\n", ptr);
		return -1;
	}
	//
	if((ptr = sb_conf_readv("audio-codec-format", buf, sizeof(buf))) == NULL) {
		sb_error("# RTSP[config]: no audio codec format specified.\n");
		return -1;
	}
	if((conf->audio_codec_format = av_get_sample_fmt(ptr)) == AV_SAMPLE_FMT_NONE) {
		sb_error("# RTSP[config]: unsupported audio codec format '%s'\n", ptr);
		return -1;
	}
	//
	if((ptr = sb_conf_readv("audio-codec-channel-layout", buf, sizeof(buf))) == NULL) {
		sb_error("# RTSP[config]: no audio codec channel layout specified.\n");
		return -1;
	}
	if((conf->audio_codec_channel_layout = av_get_channel_layout(ptr)) == 0) {
		sb_error("# RTSP[config]: unsupported audio codec channel layout '%s'\n", ptr);
		return -1;
	}
	// LAST: video-specific parameters
	if(sb_conf_mapsize("video-specific") > 0) {
		//
		sb_conf_mapreset("video-specific");
		for(	ptr = sb_conf_mapkey("video-specific", buf, sizeof(buf));
			ptr != NULL;
			ptr = sb_conf_mapnextkey("video-specific", buf, sizeof(buf))) {
			//
			char *val, valbuf[1024];
			val = sb_conf_mapvalue("video-specific", valbuf, sizeof(valbuf));
			if(val == NULL || *val == '\0')
				continue;
			conf->vso->push_back(ptr);
			conf->vso->push_back(val);
			sb_error("# RTSP[config]: video specific option: %s = %s\n",
				ptr, val);
		}
	}
	return 0;
