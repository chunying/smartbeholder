...
/**
 * @file
 * implementation: common SB functions and macros
 */
 ...
 */
long long
sb_usleep(long long interval, struct timeval *ptv) {
	long long delta;
	struct timeval tv;
	if(ptv != NULL) {
 ...
 * This in an internal function only called by \em sb_log function.
 *
 ...
 */
static void
sb_writelog(struct timeval tv, const char *s) {
	FILE *fp;
	if(sb_logfile == NULL)
		return;
	if((fp = fopen(sb_logfile, "at")) != NULL) {
...
sb_log(const char *fmt, ...) {
	...
	va_start(ap, fmt);
#ifdef ANDROID
	__android_log_vprint(ANDROID_LOG_INFO, "sb_log.native", fmt, ap);
#endif
 ...
 */
int
sb_malloc(int size, void **ptr, int *alignment) {
 ...
 */
int
sb_alignment(void *ptr, int alignto) {
	int mask = alignto - 1;
#ifdef __x86_64__
 ...
 */
int
sb_init(const char *config, const char *url) {
	srand(time(0));
	winsock_init();
#ifndef ANDROID_NO_FFMPEG
	av_register_all();
...
#endif
	if(config != NULL) {
		if(sb_conf_load(config) < 0) {
			sb_error("SB: cannot load configuration file '%s'\n", config);
			return -1;
		}
	}
	if(url != NULL) {
		if(sb_url_parse(url) < 0) {
			sb_error("SB: invalid URL '%s'\n", url);
			return -1;
		}
	}
	return 0;
...
/**
 * Deinitialize Smart Beholder: currently do nothing
 */
void
sb_deinit() {
	return;
...
sb_openlog() {
	...
	FILE *fp;
	//
	if(sb_conf_readv("logfile", fn, sizeof(fn)) == NULL)
		return;
	if((fp = fopen(fn, "at")) != NULL) {
		fclose(fp);
		sb_logfile = strdup(fn);
	}
	//
	return;
}
...
/**
 * Internal function for \em sb_save_init and \em sb_save_init_txt.
 ...
 */
static FILE *
sb_save_init_internal(const char *filename, const char *mode) {
	FILE *fp = NULL;
	if(filename != NULL) {
		fp = fopen(filename, mode);
		if(fp == NULL) {
			sb_error("save file: open %s failed.\n", filename);
		}
	}
	return fp;
}
 ...
 */
FILE *
sb_save_init(const char *filename) {
	return sb_save_init_internal(filename, "wb");
}
 ...
 */
FILE *
sb_save_init_txt(const char *filename) {
	return sb_save_init_internal(filename, "wt");
}

/**
 * Save arbitrary binary data.
 *
 * @param fp [in] FILE pointer to the opened file (by \em sb_save_init).
 * @param buffer [in] Pointer to the data memory.
 * @param size [in] Size of the data, in bytes.
 */
int
sb_save_data(FILE *fp, unsigned char *buffer, int size) {
 ...
 * Save text data.
 *
 * @param fp [in] FILE pointer to the opened file (by \em sb_save_init_txt).
 * @param fmt [in] Pointer the for format string.
 * @param ... [in] Arguments for specifiers in the format string.
 */
int
sb_save_printf(FILE *fp, const char *fmt, ...) {
	int wlen;
	va_list ap;
	if(fp == NULL)
		return -1;
 ...
 */
int
sb_save_yuv420p(FILE *fp, int w, int h, unsigned char *planes[], int linesize[]) {
	...
	//
	if(written != expected) {
		sb_error("save YUV (%dx%d): expected %d, save %d (frame may be corrupted)\n",
 ...
 */
int
sb_save_rgb4(FILE *fp, int w, int h, unsigned char *planes, int linesize) {
	...
	//
	if(written != expected) {
		sb_error("save RGB4 (%dx%d): expected %d, save %d (frame may be corrupted)\n",
			w, h, expected, written);
	}
	//
	fflush(fp);
...
/**
 * Close a file opened by \em sb_save_init or \em sb_save_init_txt
 *
 * @param fp [in] The opened file pointer.
 * @return This function currently always returns 0.
 */
int
sb_save_close(FILE *fp) {
	if(fp != NULL) {
		fclose(fp);
	}
	return 0;
 ...
 */
void
sb_aggregated_print(int key, int limit, int value) {
	...
	if(mi->second.size() >= limit) {
		...
		if(li != mi->second.end()) {
			sb_error("insufficeient space for aggregated messages.\n");
		}
		mi->second.clear();
		//
		gettimeofday(&tv, NULL);
#ifdef ANDROID
		__android_log_write(ANDROID_LOG_INFO, "sb_log.native", buf);
#else
 ...
 */
unsigned char *
sb_find_startcode(unsigned char *buf, unsigned char *end, int *startcode_len) {
	unsigned char *ptr;
...
struct gaRect *
sb_fillrect(struct gaRect *rect, int left, int top, int right, int bottom) {
	if(rect == NULL)
		return NULL;
...
#undef	SWAP
	...
	if(rect->width <= 0 || rect->height <= 0) {
		sb_error("# invalid rect size (%dx%d)\n", rect->width, rect->height);
		return NULL;
	}
	//
	return rect;
...
#ifdef WIN32
int
sb_crop_window(struct gaRect *rect, struct gaRect **prect) {
	...
	if(rect == NULL || prect == NULL)
		return -1;
	//
	pname = sb_conf_readv("find-window-name", wndname, sizeof(wndname));
	pclass = sb_conf_readv("find-window-class", wndclass, sizeof(wndclass));
	//
	...
	if((hWnd = FindWindow(pclass, pname)) == NULL) {
		sb_error("FindWindow failed for '%s/%s'\n",
			pclass ? pclass : "",
			pname ? pname : "");
		return -1;
	}
	...
	//
	sb_error("Found window (0x%08x) :%s%s%s%s\n", hWnd,
		pclass ? " class=" : "",
		pclass ? pclass : "",
		pname ? " name=" : "",
		pname ? pname : "");
	...
	if(SetWindowPos(hWnd, HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE|SWP_SHOWWINDOW) == 0) {
		sb_error("SetWindowPos failed.\n");
		return -1;
	}
	if(GetClientRect(hWnd, &client) == 0) {
		sb_error("GetClientRect failed.\n");
		return -1;
	}
	if(SetForegroundWindow(hWnd) == 0) {
		sb_error("SetForegroundWindow failed.\n");
	}
	//
	lt.x = client.left;
	lt.y = client.top;
	...
	|| ClientToScreen(hWnd, &rb) == 0) {
		sb_error("Map from client coordinate to screen coordinate failed.\n");
		return -1;
	}
	//
	rect->left = lt.x;
	...
	if(rect->left < 0 || rect->top < 0 || rect->right >= dw || rect->bottom >= dh) {
		sb_error("Invalid window: (%d,%d)-(%d,%d) w=%d h=%d (screen dimension = %dx%d).\n",
...
sb_crop_window(struct gaRect *rect, struct gaRect **prect) {
	...
	if(rect == NULL || prect == NULL)
		return -1;
	//
	if((pdisplay = sb_conf_readv("display", display, sizeof(display))) == NULL) {
		*prect = NULL;
		return 0;
	}
	if((pname = sb_conf_readv("find-window-name", wndname, sizeof(wndname))) == NULL) {
		*prect = NULL;
		return 0;
	}
	//
	if((d = XOpenDisplay(pdisplay)) == NULL) {
		sb_error("sb_crop_window: cannot open display %s\n", display);
		return -1;
	}
	...
	if((w = FindWindowX(d, RootWindow(d, screen), pname)) == 0) {
		sb_error("FindWindowX failed for %s/%s\n", pdisplay, pname);
		XCloseDisplay(d);
		return -1;
	}
	if(GetClientRectX(d, screen, w, rect) < 0) {
		sb_error("GetClientRectX failed for %s/%s\n", pdisplay, pname);
		XCloseDisplay(d);
		return -1;
	}
	XRaiseWindow(d, w);
...
/**
 * Data struture to describe codecs supported by SB.
 * The last data must have a key of NULL.
 */
struct sb_codec_entry {
 ...
 * The codec table.
 */
struct sb_codec_entry sb_codec_table[] = {
 ...
 */
static sb_codec_entry *
sb_lookup_core(const char *key) {
	int i = 0;
	while(i >= 0 && sb_codec_table[i].key != NULL) {
		if(strcasecmp(sb_codec_table[i].key, key) == 0)
			return &sb_codec_table[i];
		i++;
	}
	return NULL;
}
 ...
 */
const char *
sb_lookup_mime(const char *key) {
	struct sb_codec_entry * e = sb_lookup_core(key);
	if(e==NULL || e->mime==NULL) {
		sb_error("sb_lookup[%s]: mime not found\n", key);
		return NULL;
	}
	return e->mime;
}
 ...
 */
const char **
sb_lookup_ffmpeg_decoders(const char *key) {
	struct sb_codec_entry * e = sb_lookup_core(key);
	if(e==NULL || e->ffmpeg_decoders==NULL) {
		sb_error("sb_lookup[%s]: ffmpeg decoders not found\n", key);
		return NULL;
	}
 ...
 */
enum AVCodecID
sb_lookup_codec_id(const char *key) {
	struct sb_codec_entry * e = sb_lookup_core(key);
	if(e==NULL) {
		sb_error("sb_lookup[%s]: codec id not found\n", key);
