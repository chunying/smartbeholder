...
/*struct SwsContext *
sb_swscale_init(PixelFormat format, int inW, int inH, int outW, int outH) {
	...
	if((swsctx = sws_getContext(
		...
		fprintf(stderr, "# core-swscale-init: cannot create swscale context\n");
	}
	return swsctx;
}*/

AVFormatContext*
sb_format_init(const char *filename) {
	AVOutputFormat *fmt;
	AVFormatContext *ctx;
	//
...
AVStream *
sb_avformat_new_stream(AVFormatContext *ctx, int id, AVCodec *codec) {
	AVStream *st = NULL;
	if(codec == NULL)
		return NULL;
...
AVCodec*
sb_avcodec_find_encoder(const char **names, enum AVCodecID cid) {
	AVCodec *codec = NULL;
	if(names != NULL) {
		while(*names != NULL) {
...
AVCodec*
sb_avcodec_find_decoder(const char **names, enum AVCodecID cid) {
	AVCodec *codec = NULL;
	if(names != NULL) {
		while(*names != NULL) {
...
AVCodecContext*
sb_avcodec_vencoder_init(AVCodecContext *ctx, AVCodec *codec, int width, int height, int fps, vector<string> *vso) {
	...
	if(vso != NULL) {
		...
		for(i = 0; i < n; i += 2) {
			...
			sb_error("vencoder-init: option %s = %s\n",
				(*vso)[i].c_str(),
				(*vso)[i+1].c_str());
		}
	} else {
		sb_error("vencoder-init: using default video encoder parameter.\n");
	}
...
AVCodecContext*
sb_avcodec_aencoder_init(AVCodecContext *ctx, AVCodec *codec, int bitrate, int samplerate, int channels, AVSampleFormat format, uint64_t chlayout) {
	AVDictionary *opts = NULL;
...
void
sb_avcodec_close(AVCodecContext *ctx) {
	if(ctx == NULL)
		return;
	pthread_mutex_lock(&avcodec_open_mutex);
	avcodec_close(ctx);
