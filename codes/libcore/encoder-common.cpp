...
// list of encoders
static sb_module_t *vencoder = NULL;	/**< Video encoder instance */
static sb_module_t *aencoder = NULL;	/**< Audio encoder instance */
static sb_module_t *sinkserver = NULL;	/**< Sink server instance */
 ...
 */
int
encoder_register_vencoder(sb_module_t *m, void *param) {
	if(vencoder != NULL) {
		sb_error("encoder: warning - replace video encoder %s with %s\n",
			vencoder->name, m->name);
	}
	vencoder = m;
	vencoder_param = param;
	sb_error("video encoder: %s registered\n", m->name);
	return 0;
}
 ...
 */
int
encoder_register_aencoder(sb_module_t *m, void *param) {
	if(aencoder != NULL) {
		sb_error("encoder warning - replace audio encoder %s with %s\n",
			aencoder->name, m->name);
	}
	aencoder = m;
	aencoder_param = param;
	sb_error("audio encoder: %s registered\n", m->name);
	return 0;
}
 ...
 */
int
encoder_register_sinkserver(sb_module_t *m) {
	if(m->send_packet == NULL) {
		sb_error("encoder error: sink server %s does not define send_packet interface\n", m->name);
		return -1;
	}
	if(sinkserver != NULL) {
		sb_error("encoder warning: replace sink server %s with %s\n",
			sinkserver->name, m->name);
	}
	sinkserver = m;
	sb_error("sink server: %s registered\n", m->name);
	return 0;
}
...
encoder_register_client(void /*RTSPContext*/ *rtsp) {
	...
	if(encoder_clients.size() == 0) {
		...
		if(aencoder != NULL && aencoder->init != NULL) {
			if(aencoder->init(aencoder_param) < 0) {
				sb_error("audio encoder: init failed.\n");
				exit(-1);
			}
		}
		...
		if(vencoder != NULL && vencoder->start != NULL) {
			if(vencoder->start(vencoder_param) < 0) {
				...
				sb_error("video encoder: start failed.\n");
		...
		if(aencoder != NULL && aencoder->start != NULL) {
			if(aencoder->start(aencoder_param) < 0) {
				pthread_rwlock_unlock(&encoder_lock);
				sb_error("audio encoder: start failed.\n");
				threadLaunched = false;
				exit(-1);
			}
		}
	...
	sb_error("encoder client registered: total %d clients.\n", encoder_clients.size());
	pthread_rwlock_unlock(&encoder_lock);
	return 0;
}
...
encoder_unregister_client(void /*RTSPContext*/ *rtsp) {
	...
	sb_error("encoder client unregistered: %d clients left.\n", encoder_clients.size());
	if(encoder_clients.size() == 0) {
		threadLaunched = false;
		sb_error("encoder: no more clients, quitting ...\n");
...
encoder_send_packet(const char *prefix, int channelId, AVPacket *pkt, int64_t encoderPts, struct timeval *ptv) {
	...
	sb_error("encoder: no sink server registered.\n");
	return -1;
}
...
#if 1
	sb_error("FIXME: encoder_ptv_get failed: id=%d, pts=%lld\n", queueid, pts);
#endif
	return NULL;
}
...
encoder_pktqueue_init(int channels, int qsize) {
	...
	for(i = 0; i < channels; i++) {
		...
		if((pktqueue[i].buf = (char *) malloc(qsize)) == NULL) {
			sb_error("encoder: initialized packet queue#%d failed (%d bytes)\n",
				i, qsize);
			exit(-1);
		}
	...
	sb_error("encoder: packet queue initialized (%dx%d bytes)\n", channels, qsize);
	return 0;
}
...
size_check:
	...
	if(q->datasize + pkt->size > q->bufsize) {
		...
		sb_error("encoder: packet queue #%d full, packet dropped (%d+%d)\n",
...
encoder_pktqueue_register_callback(int channelId, qcallback_t cb) {
	...
	sb_error("encoder: pktqueue #%d callback registered (%p)\n", channelId, cb);
	return 0;
}
