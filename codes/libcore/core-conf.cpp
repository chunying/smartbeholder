...
/**
 * @file
 * Smart Beholder configuration file loader: implementations
 */
...
static map<string,gaConfVar> sb_vars;
static map<string,gaConfVar>::iterator sb_vmi = sb_vars.begin();
 ...
 */
static int
sb_conf_parse(const char *filename, int lineno, char *buf) {
	...
	*token++ = '\0';
	//
	option = sb_conf_trim(option);
	if(*option == '\0')
		return 0;
	//
	token = sb_conf_trim(token);
	if(token[0] == '\0')
		return 0;
...
#endif
		sb_error("# include: %s\n", incfile);
		return sb_conf_load(incfile);
	}
	// check if its a map
	if((leftbracket = strchr(option, '[')) != NULL) {
		rightbracket = strchr(leftbracket+1, ']');
		if(rightbracket == NULL) {
			sb_error("# %s:%d: malformed option (%s without right bracket).\n",
				filename, lineno, option);
			return -1;
		}
		// no key specified
		if(leftbracket + 1 == rightbracket) {
			sb_error("# %s:%d: malformed option (%s without a key).\n",
				filename, lineno, option);
			return -1;
		}
		// garbage after rightbracket?
		if(*(rightbracket+1) != '\0') {
			sb_error("# %s:%d: malformed option (%s?).\n",
				filename, lineno, option);
			return -1;
		}
		*leftbracket = '\0';
	...
	if(leftbracket != NULL) {
		...
		sb_vars[option][leftbracket] = token;
	} else {
		//sb_error("%s = %s\n", option, token);
		sb_vars[option] = token;
	}
	return 0;
}
 ...
 */
int
sb_conf_load(const char *filename) {
	FILE *fp;
	char buf[8192];
	int lineno = 0;
	//
	...
	while(fgets(buf, sizeof(buf), fp) != NULL) {
		lineno++;
		if(sb_conf_parse(filename, lineno, buf) < 0) {
			fclose(fp);
			return -1;
		}
	}
	...
	for(ptr = servername; *ptr; ptr++) {
		...
		if(*ptr == ':') {
			...
			*ptr = '\0';
			...
			serverport[i] = '\0';
			sb_conf_writev("server-port", serverport);
			break;
		}
	}
	sb_conf_writev("server-url", url);
	sb_conf_writev("server-name", servername);
	return 0;
}
 ...
 */
char *
sb_conf_readv(const char *key, char *store, int slen) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(key)) == sb_vars.end())
		return NULL;
 ...
 */
int
sb_conf_readint(const char *key) {
	char buf[64];
	char *ptr = sb_conf_readv(key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0;
 ...
 */
double
sb_conf_readdouble(const char *key) {
	char buf[64];
	char *ptr = sb_conf_readv(key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0.0;
 ...
 */
static int
sb_conf_multiple_int(char *buf, int *val, int n) {
	int reads = 0;
	char *endptr, *ptr = buf;
	while(reads < n) {
 ...
 */
int
sb_conf_readints(const char *key, int *val, int n) {
	char buf[1024];
	char *ptr = sb_conf_readv(key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0;
	return sb_conf_multiple_int(buf, val, n);
}
 ...
 */
int
sb_conf_boolval(const char *ptr, int defval) {
 ...
 * @return 0 for FALSE, or 1 for TRUE.
 *
 * See the definitions defined in \em sb_conf_boolval function:
 * - The following string values are treated as TRUE: true, 1, y, yes, enabled, enable.
 * - The following string values are treated as FALSE: false, 0, n, no, disabled, disable. 
 */
int
sb_conf_readbool(const char *key, int defval) {
	char buf[64];
	char *ptr = sb_conf_readv(key, buf, sizeof(buf));
	if(ptr == NULL)
		return defval;
	return sb_conf_boolval(ptr, defval);
}
 ...
 */
int
sb_conf_writev(const char *key, const char *value) {
	sb_vars[key] = value;
	return 0;
}
 ...
 */
int
sb_conf_ismap(const char *key) {
	return sb_conf_mapsize(key) > 0 ? 1 : 0;
}
 ...
 */
int
sb_conf_haskey(const char *mapname, const char *key) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return 0;
 ...
 */
int
sb_conf_mapsize(const char *mapname) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return 0;
 ...
 */
char *
sb_conf_mapreadv(const char *mapname, const char *key, char *store, int slen) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return NULL;
	if((mi->second)[key] == "")
 ...
 */
int
sb_conf_mapreadint(const char *mapname, const char *key) {
	char buf[64];
	char *ptr = sb_conf_mapreadv(mapname, key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0;
	return strtol(ptr, NULL, 0);
}
 ...
 */
int
sb_conf_mapreadints(const char *mapname, const char *key, int *val, int n) {
	char buf[1024];
	char *ptr = sb_conf_mapreadv(mapname, key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0;
 ...
 */
double
sb_conf_mapreaddouble(const char *mapname, const char *key) {
	char buf[64];
	char *ptr = sb_conf_mapreadv(mapname, key, buf, sizeof(buf));
	if(ptr == NULL)
		return 0.0;
 ...
 */
int
sb_conf_mapreadbool(const char *mapname, const char *key, int defval) {
	char buf[64];
	char *ptr = sb_conf_mapreadv(mapname, key, buf, sizeof(buf));
	if(ptr == NULL)
		return defval;
 ...
 */
int
sb_conf_mapwritev(const char *mapname, const char *key, const char *value) {
	sb_vars[mapname][key] = value;
	return 0;
}
 ...
 */
void
sb_conf_maperase(const char *mapname, const char *key) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return;
	sb_vars.erase(mi);
 ...
 */
void
sb_conf_mapreset(const char *mapname) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return;
	mi->second.mreset();
 ...
 */
char *
sb_conf_mapkey(const char *mapname, char *keystore, int klen) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return NULL;
	if(mi->second.mkey() == "")
 ...
 */
char *
sb_conf_mapvalue(const char *mapname, char *valstore, int vlen) {
	map<string,gaConfVar>::iterator mi;
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
		return NULL;
	if(mi->second.mkey() == "")
 ...
 */
char *
sb_conf_mapnextkey(const char *mapname, char *keystore, int klen) {
	map<string,gaConfVar>::iterator mi;
	string k = "";
	//
	if((mi = sb_vars.find(mapname)) == sb_vars.end())
 ...
 * This function is used to enumerate all runtime configurations.
 */
const char *sb_conf_key() {
	if(sb_vmi == sb_vars.end())
		return NULL;
	return sb_vmi->first.c_str();
}
