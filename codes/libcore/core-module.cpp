...
/**
 * @file
 * Common functions and definitions for implementing a Smart Beholder module.
 */
 ...
 */
static map<sb_module_t *, HMODULE> mlist;
 ...
 * @return The loaded module in \em sb_module_t structure.
 *
 ...
 *
 * A Smart Beholder module must implement the \em module_load() function.
 * This function has to return an instance of the \em sb_module_t,
 * which is then returned to the caller of this function.
 */
sb_module_t *
sb_load_module(const char *modname, const char *prefix) {
	char fn[1024];
	sb_module_t *m;
	HMODULE handle;
	sb_module_t * (*do_module_load)();
#ifdef WIN32
	...
	if((handle = dlopen(fn, RTLD_NOW|RTLD_LOCAL)) == NULL) {
		sb_error("sb_load_module: load module (%s) failed - %s.\n", fn, dlerror());
		return NULL;
	}
	if((do_module_load = (sb_module_t * (*)()) dlsym(handle, "module_load")) == NULL) {
		sb_error("sb_load_module: [%s] is not a valid module.\n", fn);
		dlclose(handle);
		return NULL;
	}
 ...
 */
void
sb_unload_module(sb_module_t *m) {
	map<sb_module_t *, HMODULE>::iterator mi;
	if(m == NULL)
		return;
 ...
 */
int
sb_init_single_module(const char *name, sb_module_t *m, void *arg) {
	if(m->init == NULL)
		return 0;
	if(m->init(arg) < 0) {
		sb_error("%s init failed.\n", name);
		return -1;
	}
	return 0;
}
 ...
 *	Note that the argument could be different for different types of module.
 *
 * This function calls \em sb_init_single_module, and quit the program
 * if an error is returned from that function.
 */
void
sb_init_single_module_or_quit(const char *name, sb_module_t *m, void *arg) {
	if(sb_init_single_module(name, m, arg) < 0)
		exit(-1);
	return;
}
 ...
 */
int
sb_run_single_module(const char *name, void * (*threadproc)(void*), void *arg) {
	pthread_t t;
	if(threadproc == NULL)
		return 0;
	if(pthread_create(&t, NULL, threadproc, arg) != 0) {
		sb_error("cannot create %s thread\n", name);
		return -1;
	}
	pthread_detach(t);
	return 0;
 ...
 */
void
sb_run_single_module_or_quit(const char *name, void * (*threadproc)(void*), void *arg) {
	if(sb_run_single_module(name, threadproc, arg) < 0)
		exit(-1);
	return;
}
 ...
 */
int
sb_module_init(sb_module_t *m, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->init == NULL)
		return 0;
	return m->init(arg);
}
 ...
 */
int
sb_module_start(sb_module_t *m, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->start == NULL)
		return SB_IOCTL_ERR_NOINTERFACE;
	return m->start(arg);
}
 ...
 */
int
sb_module_stop(sb_module_t *m, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->stop == NULL)
		return 0;
 ...
 */
int
sb_module_deinit(sb_module_t *m, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->deinit == NULL)
		return 0;
 ...
 */
int
sb_module_ioctl(sb_module_t *m, int command, int argsize, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->ioctl == NULL)
		return SB_IOCTL_ERR_NOIOCTL;
 ...
 */
int
sb_module_notify(sb_module_t *m, void *arg) {
	if(m == NULL)
		return SB_IOCTL_ERR_NULLMODULE;
	if(m->notify == NULL)
		return 0;
 ...
 */
void *
sb_module_raw(sb_module_t *m, void *arg, int *size) {
	if(m == NULL)
		return NULL;
	if(m->raw == NULL)
		return NULL;
 ...
 */
int
sb_module_send_packet(sb_module_t *m, const char *prefix, int channelId, AVPacket *pkt, int64_t encoderPts, struct timeval *ptv) {
