...
ctrl_queue_init(int size, int maxunit) {
	...
	sb_error("controller queue: initialized size=%d (%d units)\n",
		qsize, qsize/qunit);
	return 0;
}
...
ctrl_queue_read_msg() {
	...
	if(qbuffer == NULL) {
		pthread_mutex_unlock(&queue_mutex);
		sb_error("controller queue: buffer released.\n");
		return NULL;
	}
	if(qtail == qhead) {
		// queue is empty
...
ctrl_queue_release_msg(struct queuemsg *msg) {
	...
	if(msg != currmsg) {
		sb_error("controller queue: WARNING - release an incorrect msg?\n");
	}
	qhead += qunit;
	if(qhead == qsize) {
		qhead = 0;
...
ctrl_queue_write_msg(void *msg, int msgsize) {
	...
	if((msgsize + sizeof(struct queuemsg)) > qunit) {
		sb_error("controller queue: msg size exceeded (%d > %d).\n",
...
ctrl_socket_init(struct RTSPConf *conf) {
	...
	if(conf->ctrlproto == IPPROTO_TCP) {
		...
		sb_error("Controller socket-init: not supported protocol.\n");
		return -1;
	}
	if(ctrlsocket < 0) {
		sb_error("Controller socket-init: %s\n", strerror(errno));
	}
	//
	...
	if(conf->servername != NULL) {
		...
		if(ctrlsin.sin_addr.s_addr == INADDR_NONE) {
			sb_error("Name resolution failed: %s\n", conf->servername);
			return -1;
		}
	}
	sb_error("controller socket: socket address [%s:%u]\n",
...
ctrl_client_init(struct RTSPConf *conf, const char *ctrlid) {
	...
	if(conf->ctrlproto == IPPROTO_TCP) {
		...
		if(connect(ctrlsocket, (struct sockaddr*) &ctrlsin, sizeof(ctrlsin)) < 0) {
			sb_error("controller client-connect: %s\n", strerror(errno));
			goto error;
		}
		// send handshake
		...
		if(send(ctrlsocket, (char*) &hh, hh.length, 0) <= 0) {
			sb_error("controller client-send(handshake): %s\n", strerror(errno));
			goto error;
		}
	}
	return 0;
error:
	conf->ctrlenable = 0;
	ctrlenabled = false;
	sb_error("controller client: controller disabled.\n");
	close(ctrlsocket);
	ctrlsocket = -1;
	return -1;
}
...
ctrl_client_thread(void *rtspconf) {
	...
	if(ctrl_client_init(conf, CTRL_CURRENT_VERSION) < 0) {
		sb_error("controller client-thread: init failed, thread terminated.\n");
		return NULL;
	}

	sb_error("controller client-thread started: tid=%ld.\n", sb_gettid());
	...
	while(true) {
		...
		while((qm = ctrl_queue_read_msg()) != NULL) {
			int wlen;
			if(qm->msgsize == 0) {
				sb_error("controller client: null messgae received, terminate the thread.\n");
				goto quit;
			}
#ifdef ANDROID
			if(drop > 0)
			...
			if(conf->ctrlproto == IPPROTO_TCP) {
				if((wlen = send(ctrlsocket, (char*) qm->msg, qm->msgsize, 0)) < 0) {
					sb_error("controller client-send(tcp): %s\n", strerror(errno));
#ifdef ANDROID
					drop = 1;
#else
					exit(-1);
				...
				if((wlen = sendto(ctrlsocket, (char*) qm->msg, qm->msgsize, 0, (struct sockaddr*) &ctrlsin, sizeof(ctrlsin))) < 0) {
					sb_error("controller client-send(udp): %s\n", strerror(errno));
#ifdef ANDROID
					drop = 1;
#else
					exit(-1);
			...
			//sb_error("controller client-debug: send msg (%d bytes)\n", wlen);
		}
	}
...
quit:
	close(ctrlsocket);
	ctrlsocket = -1;
	sb_error("controller client-thread terminated: tid=%ld.\n", sb_gettid());
...
ctrl_client_sendmsg(void *msg, int msglen) {
	if(ctrlenabled == false) {
		sb_error("controller client-sendmsg: controller was disabled.\n");
		return;
	}
	if(ctrl_queue_write_msg(msg, msglen) != msglen) {
		sb_error("controller client-sendmsg: queue full, message dropped.\n");
	} else {
...
ctrl_server_init(struct RTSPConf *conf, const char *ctrlid) {
	...
	do {
		...
		if(setsockopt(ctrlsocket, SOL_SOCKET, SO_REUSEADDR, (char*) &val, sizeof(val)) < 0) {
			sb_error("controller server-bind: %s\n", strerror(errno));
			goto error;
		}
	} while(0);
	...
	if(conf->ctrlproto == IPPROTO_TCP) {
		if(listen(ctrlsocket, 16) < 0) {
			sb_error("controller server-listen: %s\n", strerror(errno));
			goto error;
		}
	}
	return 0;
...
ctrl_server_thread(void *rtspconf) {
	...
	if(ctrl_server_init(conf, CTRL_CURRENT_VERSION) < 0) {
		sb_error("controller server-thread: init failed, terminated.\n");
		exit(-1);
	}

	sb_error("controller server started: tid=%ld.\n", sb_gettid());
...
restart:
	...
	if(conf->ctrlproto == IPPROTO_TCP) {
		...
		if((socket = accept(ctrlsocket, (struct sockaddr*) &csin, &csinlen)) < 0) {
			sb_error("controller server-accept: %s.\n", strerror(errno));
			goto restart;
		}
		sb_error("controller server-thread: accepted TCP client from %s.%d\n",
			inet_ntoa(csin.sin_addr), ntohs(csin.sin_port));
		// check initial handshake message
		if((buflen = recv(socket, (char*) buf, sizeof(buf), 0)) <= 0) {
			sb_error("controller server-thread: %s\n", strerror(errno));
			close(socket);
			goto restart;
		}
		if(hh->length > buflen) {
			sb_error("controller server-thread: bad handshake length (%d > %d)\n",
				hh->length, buflen);
			close(socket);
			goto restart;
		}
		if(memcmp(myctrlid, hh->id, hh->length-1) != 0) {
			sb_error("controller server-thread: mismatched protocol version (%s != %s), length = %d\n",
				hh->id, myctrlid, hh->length-1);
			close(socket);
			goto restart;
		}
		sb_error("controller server-thread: receiving events ...\n");
		clientaccepted = 1;
	}
...
tcp_readmore:
			if((rlen = recv(socket, (char*) buf+buflen, sizeof(buf)-buflen, 0)) <= 0) {
				sb_error("controller server-read: %s\n", strerror(errno));
				sb_error("controller server-thread: conenction closed.\n");
				close(socket);
				goto restart;
			}
#if 0
			sb_error("controller server-debug: recv msg (%d bytes) | head = %02x %02x\n",
				rlen, buf[0], buf[1]);
#endif
			buflen += rlen;
		} else if(conf->ctrlproto == IPPROTO_UDP) {
			...
			if(clientaccepted == 0) {
				...
				sb_error("controller server-thread: NOTICE - UDP client reconnected?\n");
				bcopy(&xsin, &csin, sizeof(csin));
				//continue;
			}
		}
tcp_again:
		...
		//
		if(msglen == 0) {
			sb_error("controller server: WARNING - invalid message with size equal to zero!\n");
			continue;
		}
		...
		if(conf->ctrlproto == IPPROTO_TCP) {
			...
			if(buflen != msglen) {
				sb_error("controller server: UDP msg size matched (expected %d, got %d).\n",
					msglen, buflen);
				continue;
			}
		}
		...
		if(ctrlsys_handle_message(buf+bufhead, msglen) != 0) {
			...
			sb_error("controller server: queue full, message dropped.\n");
		} else {
			pthread_cond_signal(&wakeup);
		}
