...
static map<struct vconvcfg, struct SwsContext *> sb_converters;
...
lookup_frame_converter_internal(struct vconvcfg *ccfg) {
	...
	//
	if((mi = sb_converters.find(*ccfg)) != sb_converters.end()) {
		return mi->second;
	}
	//
	return NULL;
...
create_frame_converter(int srcw, int srch, AVPixelFormat srcfmt,
	...
	sb_converters[ccfg] = ctx;
	sb_error("Frame converter created: from (%d,%d)[%d] -> (%d,%d)[%d]\n",
