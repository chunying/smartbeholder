...
vsource_frame_init(int channel, vsource_frame_t *frame) {
	...
	frame->imgbuf += sb_alignment(frame->imgbuf, VSOURCE_ALIGNMENT);
	//sb_error("XXX: frame=%p, imgbuf=%p, sizeof(vframe)=%d, bzero(%d)\n",
...
vsource_embed_colorcode_init(int RGBmode) {
	...
	// read param
	if(sb_conf_readints("embed-colorcode", param, 3) != 3)
		return -1;
	if(sb_conf_readv("save-colorcode-timestamp", savefile_ccodets, sizeof(savefile_ccodets)) != NULL)
		savefp_ccodets = sb_save_init_txt(savefile_ccodets);
	...
	//
	sb_error("video source: color code initialized - %dx%d, %d digits, shift=%d, initmask=%08x, totalwidth=%d, counter-mask=%08x\n",
...
vsource_embed_yuv_code(vsource_frame_t *frame, unsigned int value) {
	...
	if(savefp_ccodets != NULL) {
		...
		sb_save_printf(savefp_ccodets, "COLORCODE-TIMESTAMP: %08u -> %u.%06u\n",
...
vsource_embed_rgba_code(vsource_frame_t *frame, unsigned int value, unsigned char color[8][4]) {
	...
	if(savefp_ccodets != NULL) {
		gettimeofday(&ccodets, NULL);
		sb_save_printf(savefp_ccodets, "COLORCODE-TIMESTAMP: %u -> %u.%06u\n",
			value, ccodets.tv_sec, ccodets.tv_usec);
	}
	//// make the color code line
	// compute crc
...
video_source_setup_ex(vsource_config_t *config, int nConfig) {
	...
	if(config==NULL || nConfig <=0 || nConfig > VIDEO_SOURCE_CHANNEL_MAX) {
		sb_error("video source: invalid video source configuration request=%d; MAX=%d; config=%p\n",
			nConfig, VIDEO_SOURCE_CHANNEL_MAX, config);
		return -1;
	}
	//
	if(sb_conf_readints("max-resolution", maxres, 2) != 2) {
		maxres[0] = maxres[1] = 0;
	}
	if(sb_conf_readints("output-resolution", outres, 2) != 2) {
		outres[0] = outres[1] = 0;
	}
	//
	for(idx = 0; idx < nConfig; idx++) {
		...
		if(video_source_add_pipename_internal(vs, pipename) == NULL) {
			sb_error("video source: setup pipename failed (%s).\n", pipename);
			return -1;
		}
		...
		if(gPipe[idx] == NULL) {
			sb_error("video source: init pipeline failed.\n");
			return -1;
		}
		for(data = gPipe[idx]->in; data != NULL; data = data->next) {
			if(vsource_frame_init(idx, (vsource_frame_t*) data->pointer) == NULL) {
				sb_error("video source: init faile failed.\n");
				return -1;
			}
		}
		//
		sb_error("video-source: %s initialized max-curr-out = (%dx%d)-(%dx%d)-(%dx%d)\n",
