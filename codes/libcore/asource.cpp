...
audio_source_buffer_init() {
	...
	|| channels == 0
	|| bitspersample == 0) {
		sb_error("audio source: invalid argument (frames=%d, channels=%d, bitspersample=%d)\n",
...
retry:
	...
	if(framesize > headspace + tailspace) {
		sb_error("Audio source: buffer overflow, packet dropped (%d frames)\n", frames);
...
audio_source_buffer_purge(audio_buffer_t *ab) {
	sb_error("audio: buffer purged (%d bytes / %d frames).\n",
