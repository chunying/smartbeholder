...
block_until_port_changed(OMX_HANDLETYPE hComponent, OMX_U32 nPortIndex, OMX_BOOL bEnabled) {
	...
	while(i++ == 0 || portdef.bEnabled != bEnabled) {
		if((r = OMX_GetParameter(hComponent, OMX_IndexParamPortDefinition, &portdef)) != OMX_ErrorNone) {
			sb_error("streamer: failed to get port definition (err=%x)\n", r);
			exit(-1);
		}
...
init_component_handle(const char *name, OMX_HANDLETYPE* hComponent, OMX_PTR pAppData, OMX_CALLBACKTYPE* callbacks) {
	...
	if((r = OMX_GetHandle(hComponent, fullname, pAppData, callbacks)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get handle for component %s (err=%x)\n", fullname, r);
		exit(-1);
	}
	...
	for(i = 0; i < 4; i++) {
		if(OMX_GetParameter(*hComponent, types[i], &ports) == OMX_ErrorNone) {
			...
			for(nPortIndex = ports.nStartPortNumber; nPortIndex < ports.nStartPortNumber + ports.nPorts; nPortIndex++) {
				//sb_error("streamer: disabling port %d of component %s\n", nPortIndex, fullname);
				if((r = OMX_SendCommand(*hComponent, OMX_CommandPortDisable, nPortIndex, NULL)) != OMX_ErrorNone) {
					sb_error("streamer: failed to disable port %d of component %s\n", nPortIndex, fullname);
					exit(-1);
				}
...
dump_event(OMX_HANDLETYPE hComponent, OMX_EVENTTYPE eEvent, OMX_U32 nData1, OMX_U32 nData2) {
	...
	sb_error("*** Received event 0x%08x %s, hComponent:0x%08x, nData1:0x%08x, nData2:0x%08x\n",
...
event_handler(
	...
	case OMX_EventParamOrConfigChanged:
		...
		break;
	case OMX_EventError:
		sb_error("error event received (err=%x)\n", nData1);
		exit(-1);
		break;
	default:
		break;
...
omx_streamer_deinit_streamer(omx_streamer_t *ctx) {
	...
	if((r = OMX_FillThisBuffer(ctx->encoder, ctx->outbuf)) != OMX_ErrorNone) {
		sb_error("streamer: failed to request filling of the output buffer on encoder output port 201\n");
	}
	// Flush the buffers on each component
flush_port_73:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandFlush, 73, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of camera input port 73\n");
		goto flush_port_70;
	}
	block_until_flushed(ctx);
flush_port_70:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandFlush, 70, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of camera preview output port 70\n");
		goto flush_port_71;
	}
	block_until_flushed(ctx);
flush_port_71:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandFlush, 71, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of camera video output port 71\n");
		goto flush_port_200;
	}
	block_until_flushed(ctx);
flush_port_200:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandFlush, 200, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of encoder input port 200\n");
		goto flush_port_201;
	}
	block_until_flushed(ctx);
flush_port_201:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandFlush, 201, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of encoder output port 201\n");
		goto flush_port_240;
	}
	block_until_flushed(ctx);
flush_port_240:
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandFlush, 240, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to flush buffers of null sink input port 240\n");
...
disable_port_73:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortDisable, 73, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable camera input port 73\n");
...
disable_port_70:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortDisable, 70, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable camera preview output port 70\n");
...
disable_port_71:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortDisable, 71, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable camera video output port 71\n");
...
disable_port_200:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandPortDisable, 200, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable encoder input port 200\n");
...
disable_port_201:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandPortDisable, 201, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable encoder output port 201\n");
...
disable_port_240:
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandPortDisable, 240, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to disable null sink input port 240\n");
		goto free_inbuf;
	}
...
free_inbuf:
	...
	if((r = OMX_FreeBuffer(ctx->camera, 73, ctx->inbuf)) != OMX_ErrorNone) {
		sb_error("streamer: failed to free buffer for camera input port 73\n");
	}
free_outbuf:
	if(ctx->outbuf == NULL)
		goto switch_camera_to_idle;
	if((r = OMX_FreeBuffer(ctx->encoder, 201, ctx->outbuf)) != OMX_ErrorNone) {
		sb_error("streamer: failed to free buffer for encoder output port 201\n");
	}
...
switch_camera_to_idle:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandStateSet, OMX_StateIdle, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the camera component to idle\n");
...
switch_encoder_to_idle:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandStateSet, OMX_StateIdle, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the encoder component to idle\n");
...
switch_null_sink_to_idle:
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandStateSet, OMX_StateIdle, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the null sink component to idle\n");
...
switch_camera_to_loaded:
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandStateSet, OMX_StateLoaded, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the camera component to loaded\n");
...
switch_encoder_to_loaded:
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandStateSet, OMX_StateLoaded, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the encoder component to loaded\n");
...
switch_null_sink_to_loaded:
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandStateSet, OMX_StateLoaded, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the null sink component to loaded\n");
		goto free_camera;
	}
...
free_camera:
	...
	if((r = OMX_FreeHandle(ctx->camera)) != OMX_ErrorNone) {
		sb_error("streamer: failed to free camera component handle\n");
	}
free_encoder:
	if(ctx->encoder == NULL)
		goto free_null_sink;
	if((r = OMX_FreeHandle(ctx->encoder)) != OMX_ErrorNone) {
		sb_error("streamer: failed to free encoder component handle\n");
	}
free_null_sink:
	if(ctx->null_sink == NULL)
		goto delete_semaphore;
	if((r = OMX_FreeHandle(ctx->null_sink)) != OMX_ErrorNone) {
		sb_error("streamer: failed to free null sink component handle\n");
	}
delete_semaphore:
...
omx_streamer_init_streamer(omx_streamer_t *ctx, omx_streamer_config_t *config,
	...
	if(ctx->buffer == NULL) {
		sb_error("streamer: buffer allocation failed (%d byets).\n", ctx->bufsize);
		bzero(ctx, sizeof(omx_streamer_t));
		return -1;
	}
	//
	if(vcos_semaphore_create(&ctx->handler_lock, "handler_lock", 1) != VCOS_SUCCESS) {
		sb_error("streamer: failed to create handler lock semaphore\n");
		free(ctx->buffer);
		bzero(ctx, sizeof(omx_streamer_t));
		return -1;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigRequestCallback, &cbtype)) != OMX_ErrorNone) {
		sb_error("streamer: failed to request camera device number parameter change callback for camera\n");
		goto failed;
	}
	...
	if((r = OMX_SetParameter(ctx->camera, OMX_IndexParamCameraDeviceNumber, &device)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera parameter device number\n");
		goto failed;
	}
	...
	if((r = OMX_GetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get port definition for camera preview output port 70\n");
		goto failed;
	}
	...
	if((r = OMX_SetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set port definition for camera preview output port 70\n");
		goto failed;
	}
	...
	if((r = OMX_SetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set port definition for camera video output port 71\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigVideoFramerate, &framerate)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set framerate configuration for camera preview output port 70\n");
		goto failed;
	}
	framerate.nPortIndex = 71;
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigVideoFramerate, &framerate)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set framerate configuration for camera video output port 71\n");
		goto failed;
	}
	// Configure sharpness
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonSharpness, &sharpness)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera sharpness configuration\n");
		goto failed;
	}
	// Configure contrast
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonContrast, &contrast)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera contrast configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonSaturation, &saturation)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera saturation configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonBrightness, &brightness)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera brightness configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonExposureValue, &exposure_value)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera exposure value configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonFrameStabilisation, &frame_stabilisation_control)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera frame frame stabilisation control configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonWhiteBalance, &white_balance_control)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera frame white balance control configuration\n");
		goto failed;
	}
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonImageFilter, &image_filter)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set camera image filter configuration\n");
		goto failed;
	}
	// Configure mirror
	...
	if((r = OMX_SetConfig(ctx->camera, OMX_IndexConfigCommonMirror, &mirror)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set mirror configuration for camera video output port 71\n");
		goto failed;
	}
	...
	sb_error("streamer: camera ready!\n");
#endif
	...
	if((r = OMX_GetParameter(ctx->encoder, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get port definition for encoder output port 201\n");
		goto failed;
	}
	...
	if((r = OMX_SetParameter(ctx->encoder, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set port definition for encoder output port 201\n");
		goto failed;
	}
	// Configure bitrate
	...
	if((r = OMX_SetParameter(ctx->encoder, OMX_IndexParamVideoBitrate, &bitratetype)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set bitrate for encoder output port 201\n");
		goto failed;
	}
	// Configure format
	...
	if((r = OMX_SetParameter(ctx->encoder, OMX_IndexParamVideoPortFormat, &format)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set video format for encoder output port 201\n");
		goto failed;
	}
	// get avctype
	...
	if((r = OMX_GetParameter(ctx->encoder, OMX_IndexParamVideoAvc, &avctype)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get avctype for encoder output port 201\n");
		goto failed;
	}
	...
	if((r = OMX_SetParameter(ctx->encoder, OMX_IndexParamVideoAvc, &avctype)) != OMX_ErrorNone) {
		sb_error("streamer: failed to set avctype for encoder output port 201\n");
		goto failed;
	}
	// get again and then show
	if((r = OMX_GetParameter(ctx->encoder, OMX_IndexParamVideoAvc, &avctype)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get avctype for encoder output port 201 (again)\n");
		goto failed;
	}
	sb_error("streamer: AVC config - nSliceHeaderSpacing=%d nPFrames=%d nBFrames=%d nRefFrames=%d UEP=%d FMO=%d ASO=%d RS=%d Profile=0x%x Level=0x%x AllowedFrames=0x%x\n",
	...
	if((r = OMX_SetupTunnel(ctx->camera, 70, ctx->null_sink, 240)) != OMX_ErrorNone) {
		sb_error("streamer: failed to setup tunnel between camera preview output port 70 and null sink input port 240\n");
		goto failed;
	}
	if((r = OMX_SetupTunnel(ctx->camera, 71, ctx->encoder, 200)) != OMX_ErrorNone) {
		sb_error("streamer: failed to setup tunnel between camera video output port 71 and encoder input port 200\n");
		goto failed;
	}
#endif
#if 1	//////// switch states
	...
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortEnable, 73, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable camera input port 73\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortEnable, 70, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable camera preview output port 70\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandPortEnable, 71, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable camera video output port 71\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandPortEnable, 200, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable encoder input port 200\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandPortEnable, 201, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable encoder output port 201\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandPortEnable, 240, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to enable null sink input port 240\n");
		goto failed;
	}
	...
	if((r = OMX_GetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
		sb_error("streamer: failed to get port definition for camera input port 73\n");
		goto failed;
	}
	if((r = OMX_AllocateBuffer(ctx->camera, &ctx->inbuf, 73, NULL, camera_portdef.nBufferSize)) != OMX_ErrorNone) {
		sb_error("streamer: failed to allocate buffer for camera input port 73\n");
		goto failed;
	}
	OMX_INIT_STRUCTURE(encoder_portdef);
	encoder_portdef.nPortIndex = 201;
	...
	if((r = OMX_AllocateBuffer(ctx->encoder, &ctx->outbuf, 201, NULL, encoder_portdef.nBufferSize)) != OMX_ErrorNone) {
		sb_error("streamer: failed to allocate buffer for encoder output port 201\n");
		goto failed;
	}
#endif
#if 1	/////// switch to executing state
	if((r = OMX_SendCommand(ctx->camera, OMX_CommandStateSet, OMX_StateExecuting, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the camera component to executing\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->encoder, OMX_CommandStateSet, OMX_StateExecuting, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the encoder component to executing\n");
		goto failed;
	}
	...
	if((r = OMX_SendCommand(ctx->null_sink, OMX_CommandStateSet, OMX_StateExecuting, NULL)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch state of the null sink component to executing\n");
		goto failed;
	}
	block_until_state_changed(ctx->null_sink, OMX_StateExecuting);
#endif
	//
	sb_error("streamer: initialized\n");
	//
	return 0;
failed:
...
omx_streamer_prepare_stop(omx_streamer_t *ctx) {
	...
	int frame_out = 0;
	//
	sb_error("streamer: prepare stop - wating for SYNC frame ...\n");
	while(1) {
		if(ctx->encoder_output_buffer_available) {
			...
			if(prepare_stop && (one_more_syncframe ^ (ctx->outbuf->nFlags & OMX_BUFFERFLAG_SYNCFRAME))) {
				sb_error("streamer: prepare stop - SYNC frame detected (%d), terminating ...\n", frame_out);
				break;
			}
		}
		if(ctx->encoder_output_buffer_available || ctx->buffer_filled==0) {
			...
			if((r = OMX_FillThisBuffer(ctx->encoder, ctx->outbuf)) != OMX_ErrorNone) {
				sb_error("streamer: prepare stop - failed to request filling of the output buffer on encoder output port 201\n");
				return -1;
			}
		}
		usleep(2000);
...
omx_streamer_dummy_encode(omx_streamer_t *ctx) {
	...
	if(omx_streamer_start(ctx) < 0) {
		sb_error("streamer: start streamer failed (dummy encode)\n");
		return -1;
	}
	//
	while((ctx->spslen == 0 || ctx->ppslen==0) && ctx->frame_out < 16) {
				...
				if(offset > 0) {
					if((dat[offset] & 0x1f) == 7) {
						...
						if(ctx->spslen < sizeof(ctx->sps)) {
							...
							sb_error("streamer: sps too long (%d)\n", ctx->spslen);
							ctx->spslen = 0;
						}
						...
						if(ctx->ppslen < sizeof(ctx->pps)) {
							...
							sb_error("streamer: pps too long (%d)\n", ctx->ppslen);
							ctx->ppslen = 0;
						}
					}
				}
		...
		if(ctx->encoder_output_buffer_available || ctx->buffer_filled==0) {
			...
			ctx->encoder_output_buffer_available = 0;
			if((r = OMX_FillThisBuffer(ctx->encoder, ctx->outbuf)) != OMX_ErrorNone) {
				sb_error("streamer: failed to request filling of the output buffer on encoder output port 201 (dummy encode)\n");
				return -1;
			}
		}
		usleep(2000);
	...
	if(ctx->spslen == 0 || ctx->ppslen == 0) {
		sb_error("streamer: dummy encode failed to retrieve sps/pps (sps=%d, pps=%d).\n",
			ctx->spslen, ctx->ppslen);
		return -1;
	}
	sb_error("streamer: dummy encode completed [%d output frame(s), sps=%d, pps=%d]\n",
...
omx_streamer_init(omx_streamer_t *ctx, omx_streamer_config_t *config, int width, int height, int fps_n, int fps_d, int bitrate, int gopsize) {
	...
	sb_error("streamer: encoder initialized (initial-phase).\n");
	...
	sb_error("streamer: resetting streamer ...\n");
	omx_streamer_deinit_streamer(ctx);
	sb_error("streamer: reinitializing ...\n");
	//
	if(omx_streamer_init_streamer(ctx, config, width, height, fps_n, fps_d, bitrate, gopsize) < 0) {
		return -1;
	}
	...
	sb_error("streamer: codec configuration preserved (sps=%d, pps=%d).\n", ctx->spslen, ctx->ppslen);
	sb_error("streamer: encoder initialized (final-phase).\n");
	ctx->initialized = 1;
	//
	return 0;
}
...
omx_streamer_suspend(omx_streamer_t *ctx) {
	...
	if(omx_streamer_prepare_stop(ctx) < 0) {
		sb_error("streamer[suspend]: prepare stop failed.\n");
		goto failed;
	}
	if(omx_streamer_stop(ctx) < 0) {
		sb_error("streamer[suspend]: stop failed.\n");
		goto failed;
	}
	omx_streamer_deinit_streamer(ctx);
	psuspend = &suspended;
...
omx_streamer_resume(omx_streamer_t *ctx) {
	...
	if(omx_streamer_init_streamer(ctx, &psuspend->config,
		...
		sb_error("streamer[resume]: init failed.\n");
		goto failed;
	}
	if(omx_streamer_start(ctx) < 0) {
		sb_error("streamer[resume]: start failed.\n");
		goto failed;
	}
	psuspend = NULL;
	pthread_mutex_unlock(&reconf_mutex);
...
omx_streamer_reconfigure(omx_streamer_t *ctx, int bitrateKbps, unsigned int framerate, unsigned int width, unsigned int height) {
	...
	/////////////////////////////////////////////////////
	if(omx_streamer_prepare_stop(ctx) < 0) {
		sb_error("streamer[reconfigure]: prepare stop failed.\n");
		goto failed;
	}
	if(omx_streamer_stop(ctx) < 0) {
		sb_error("streamer[reconfigure]: stop failed.\n");
		goto failed;
	}
	omx_streamer_deinit_streamer(ctx);
	if(omx_streamer_init_streamer(ctx, &old_config,
			old_width, old_height, fps_n, fps_d, bitrate, old_gopsize) < 0) {
		sb_error("streamer[reconfigure]: init failed.\n");
		goto failed;
	}
	if(omx_streamer_start(ctx) < 0) {
		sb_error("streamer[reconfigure]: start failed.\n");
		goto failed;
	}
	pthread_mutex_unlock(&reconf_mutex);
	return 0;
	...
	camera_portdef.nPortIndex = 70;
	if((r = OMX_GetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
		sb_error("streamer[reconfigure]: failed to get port definition for camera preview output port 70\n");
		goto failed;
	}
	if(framerate != 0) {
		camera_portdef.format.video.xFramerate = framerate;
		if((r = OMX_SetParameter(ctx->camera, OMX_IndexParamPortDefinition, &camera_portdef)) != OMX_ErrorNone) {
			sb_error("streamer[reconfigure]: failed to set port definition for camera preview output port 70\n");
			goto failed;
		}
	}
	...
	encoder_portdef.nPortIndex = 201;
	if((r = OMX_GetParameter(ctx->encoder, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
		sb_error("streamer[reconfigure]: failed to get port definition for encoder output port 201\n");
		goto failed;
	}
	// Copy some of the encoder output port configuration
	// from camera output port
	...
	if((r = OMX_SetParameter(ctx->encoder, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
		sb_error("streamer[reconfigure]: failed to set port definition for encoder output port 201\n");
		goto failed;
	}
	// Configure bitrate
	OMX_VIDEO_PARAM_BITRATETYPE bitratetype;
...
omx_streamer_start_stop(omx_streamer_t *ctx, OMX_BOOL start) {
	...
	if((r = OMX_SetParameter(ctx->camera, OMX_IndexConfigPortCapturing, &capture)) != OMX_ErrorNone) {
		sb_error("streamer: failed to switch %s capture on camera video output port 71\n",
			start == OMX_TRUE ? "on" : "off");
		return -1;
	}
	sb_error("streamer: camera %s\n", start == OMX_TRUE ? "started" : "stopped");
	return 0;
}
		...
		if(ctx->encoder_output_buffer_available) {
			...
			if(ctx->outbuf->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
				if(ctx->outbuf->nFilledLen <= left) {
					...
					sb_error("streamer: codec data too large (%d)\n", ctx->outbuf->nFilledLen);
				}
				end_of_frame = 1;
				goto fill_next;
			}
			// save regular data
			if(ctx->outbuf->nFilledLen > left) {
				sb_error("streamer: insufficient encoding buffer (%d bytes).\n", ctx->bufsize);
				goto fill_next;
			}
			bcopy(ctx->outbuf->pBuffer + ctx->outbuf->nOffset, dst, ctx->outbuf->nFilledLen);
			dst += ctx->outbuf->nFilledLen;
...
fill_next:
	...
	if(encsize == NULL) {
		sb_error("streamer: no encsize given\n");
		pthread_mutex_unlock(&reconf_mutex);
		return NULL;
	}
...
main(int argc, char *argv[]) {
	...
	//
	if(argc < 4) {
		sb_error("usage: %s width height fps [output.264]\n", argv[0]);
		return -1;
	}
	//
	...
	while(quit_flag == 0) {
		...
		if((encbuf = omx_streamer_get(&omx, &encsize)) == NULL) {
			sb_error("streamer: encode failed.\n");
			break;
		}
		if(fout)
	...
	sb_error("streaming captured %d frames in %.4f seconds (%.4f fps).\n",
		omx.frame_out,
		0.000001 * elapsed,
