...
static int
sb_omx_load_int(const char *name, int min, int max, int def) {
	char *ptr, value[64];
	int rv;
	if((ptr = sb_conf_readv(name, value, sizeof(value))) == NULL)
		return def;
...
static OMX_BOOL
sb_omx_load_bool(const char *name, OMX_BOOL def) {
	char *ptr, value[64];
	if(sb_conf_readbool(name, def == OMX_TRUE ? 1 : 0) == 0)
		return OMX_FALSE;
	return OMX_TRUE;
}
