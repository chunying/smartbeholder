...
liveserver_main(void *arg) {
	...
	if (rtspServer == NULL) {
		...
		sb_error("Failed to create RTSP server: %s\n", env->getResultMsg());
		exit(1);
	}
	//
	...
	ServerMediaSession * sms
		= ServerMediaSession::createNew(*env,
				rtspconf->object[0] ? &rtspconf->object[1] : "sb",
				rtspconf->object[0] ? &rtspconf->object[1] : "sb",
				rtspconf->title[0] ? rtspconf->title : "Smart Beholder Server");
	//
	qos_server_init();
	// add video session
	if((m = encoder_get_vencoder()) == NULL) {
		sb_error("live-server: FATAL - no video encoder registered.\n");
		exit(-1);
	}
	if(m->mimetype == NULL) {
		sb_error("live-server: FATAL - video encoder does not configure mimetype.\n");
		exit(-1);
	}
	for(cid = 0; cid < video_source_channels(); cid++) {
		sms->addSubsession(SBMediaSubsession::createNew(*env, cid, m->mimetype)); 
	}
	// add audio session, if necessary
	if((m = encoder_get_aencoder()) != NULL) {
		if(m->mimetype == NULL) {
			sb_error("live-server: FATAL - audio encoder does not configure mimetype.\n");
			exit(-1);
		}
		sms->addSubsession(SBMediaSubsession::createNew(*env, cid, m->mimetype)); 
	}
	...
	|| rtspServer->setUpTunnelingOverHTTP(8080)) {
		sb_error("(Use port %d for optional RTSP-over-HTTP tunneling.)\n",
			rtspServer->httpServerPortNum());
		//*env << "\n(We use port " << rtspServer->httpServerPortNum() << " for optional RTSP-over-HTTP tunneling.)\n";
	} else {
		sb_error("(RTSP-over-HTTP tunneling is not available.)\n");
	...
	for(mi = sinkmap.begin(); mi != sinkmap.end(); mi++) {
		...
		while((stats = statsIter.next()) != NULL) {
			...
			if(d_pkt_lost > d_pkt_sent) {
				sb_error("qos: invalid packet loss count detected (%d)\n", d_pkt_lost);
				d_pkt_lost = 0;
			}
			// nothing updated?
			if(d_pkt_lost == 0 && d_pkt_sent == 0 && d_byte_sent == 0)
				continue;
			// report
			sb_error("%s-report: %lldKB sent; pkt-loss=%lld/%lld,%.2f%%; bitrate=%.0fKbps; rtt=%u (%.3fms); jitter=%u (freq=%uHz)\n",
...
qos_server_add_sink(const char *prefix, RTPSink *rtpsink) {
	...
	sinkmap[rtpsink] = x;
	sb_error("qos: add sink#%d for %s, rtpsink=%p\n", sinkmap.size(), prefix, rtpsink);
	return 0;
}
...
qos_server_deinit() {
	...
	qos_task = NULL;
	sinkmap.clear();
	sb_error("qos-measurement: deinitialized.\n");
	return 0;
}
...
qos_server_init() {
	if(env == NULL) {
		sb_error("liveserver: UsageEnvironment has not been initialized.\n");
		return -1;
	}
	sinkmap.clear();
	sb_error("qos-measurement: initialized.\n");
	return 0;
}
