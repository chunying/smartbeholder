...
static SBVideoLiveSource *vLiveSource[VIDEO_SOURCE_CHANNEL_MAX];
static EventTriggerId eventTriggerId[VIDEO_SOURCE_CHANNEL_MAX];
static void signalNewVideoFrameData(int channelId);

//EventTriggerId SBVideoLiveSource::eventTriggerId = 0;
unsigned SBVideoLiveSource::referenceCount = 0;
int SBVideoLiveSource::remove_startcode = 0;
sb_module_t * SBVideoLiveSource::m = NULL;

SBVideoLiveSource * SBVideoLiveSource
::createNew(UsageEnvironment& env, int cid/* TODO: more params */) {
	return new SBVideoLiveSource(env, cid);
}
...
void SBVideoLiveSource
::deliverFrame0(void* clientData) {
	((SBVideoLiveSource*) clientData)->deliverFrame();
}
...
::deliverFrame() {
	...
	if (newFrameSize > fMaxSize) {
		...
		sb_error("video encoder: packet truncated (%d > %d).\n", newFrameSize, fMaxSize);
...
signalNewVideoFrameData(int channelId) {
	...
	SBVideoLiveSource* ourDevice = vLiveSource[channelId]; //%%% TO BE WRITTEN %%%
