...
static SBAudioLiveSource *aLiveSource = NULL;
static void signalNewAudioFrameData(int channelId);

EventTriggerId SBAudioLiveSource::eventTriggerId = 0;
unsigned SBAudioLiveSource::referenceCount = 0;

SBAudioLiveSource * SBAudioLiveSource
::createNew(UsageEnvironment& env, int cid/* TODO: more params */) {
	return new SBAudioLiveSource(env, cid);
}
...
void SBAudioLiveSource
::deliverFrame0(void* clientData) {
	((SBAudioLiveSource*) clientData)->deliverFrame();
}
...
signalNewAudioFrameData(int channelId) {
	...
	SBAudioLiveSource* ourDevice = aLiveSource; //%%% TO BE WRITTEN %%%

	if (ourScheduler != NULL) { // sanity check
		ourScheduler->triggerEvent(SBAudioLiveSource::eventTriggerId, ourDevice);
	}
}
