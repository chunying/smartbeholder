...
#include "sb-liveserver.h"
#include "sb-mediasubsession.h"
#include "sb-audiolivesource.h"
#include "sb-videolivesource.h"
#include "sb-qossink.h"

SBMediaSubsession
::SBMediaSubsession(UsageEnvironment &env, int cid, const char *mimetype, portNumBits initialPortNum, Boolean multiplexRTCPWithRTP)
		: OnDemandServerMediaSubsession(env, True/*reuseFirstSource*/, initialPortNum, multiplexRTCPWithRTP) {
	this->mimetype = strdup(mimetype);
	this->channelId = cid;
}

SBMediaSubsession * SBMediaSubsession
::createNew(UsageEnvironment &env, int cid, const char *mimetype,
		portNumBits initialPortNum,
		Boolean multiplexRTCPWithRTP) {
	return new SBMediaSubsession(env, cid, mimetype, initialPortNum, multiplexRTCPWithRTP);
}

FramedSource* SBMediaSubsession
::createNewStreamSource(unsigned clientSessionId,
	...
	if(strncmp("audio/", this->mimetype, 6) == 0) {
		...
		result = SBAudioLiveSource::createNew(envir(), this->channelId);
	} else if(strncmp("video/", this->mimetype, 6) == 0) {
		//estBitrate = 500; /* Kbps */
		estBitrate = sb_conf_mapreadint("video-specific", "b") / 1000; /* Kbps */
		OutPacketBuffer::increaseMaxSizeTo(8000000);
		result = SBVideoLiveSource::createNew(envir(), this->channelId);
	}
	do if(result != NULL) {
...
::createNewRTPSink(Groupsock* rtpGroupsock,
	...
	if(strcmp(mimetype, "audio/MPEG") == 0) {
		...
		sb_error("SBMediaSubsession: %s NOT IMPLEMENTED\n", mimetype);
		exit(-1);
		...
		sb_module_t *m = encoder_get_vencoder();
		sb_ioctl_buffer_t mb;
		...
		mb.ptr = SPS;
		mb.size = sizeof(SPS);
		if((err = sb_module_ioctl(m, SB_IOCTL_GETSPS, sizeof(mb), &mb)) < 0) {
			sb_error("unable to get SPS from %s, err=%d\n", m->name, err);
			exit(-1);
		}
		SPSSize = mb.size;
		//
		...
		mb.ptr = PPS;
		mb.size = sizeof(PPS);
		if((err = sb_module_ioctl(m, SB_IOCTL_GETPPS, sizeof(mb), &mb)) < 0) {
			sb_error("unable to get PPS from %s, err=%d\n", m->name, err);
			exit(-1);
		}
		PPSSize = mb.size;
		//
		...
		sb_error("SBMediaSubsession: %s SPS=%p(%d); PPS=%p(%d); profile_level_id=%x\n",
			mimetype,
		...
		mb.ptr = VPS;
		mb.size = sizeof(VPS);
		if((err = sb_module_ioctl(m, SB_IOCTL_GETVPS, sizeof(mb), &mb)) < 0) {
			sb_error("unable to get VPS from %s, err=%d\n", m->name, err);
			exit(-1);
		}
		VPSSize = mb.size;
		//
		sb_error("SBMediaSubsession: %s SPS=%p(%d); PPS=%p(%d); VPS=%p(%d)\n",
			mimetype,
	...
	if(result == NULL) {
		sb_error("SBMediaSubsession: create RTP sink for %s failed.\n", mimetype);
	}
	return result;
}
